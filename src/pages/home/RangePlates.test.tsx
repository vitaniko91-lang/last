...
describe('RangePlates', () => {
  it('девять плит — девять ссылок на товар, в имени есть цена', () => {
    ...
    // Цена форматируется Intl с узким неразрывным пробелом — сравниваем через formatUah, а не литералом.
    expect(screen.getByRole('link', { name: new RegExp(`Крем-реноватор.*${formatUah(990)}`, 's') })).toHaveAttribute('href', '/product/cream')
  })
